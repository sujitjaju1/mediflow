...
import { ReceptionistIntakeCard } from "@/src/components/receptionist/ReceptionistIntakeCard";
...
export default async function ReceptionistPatientsPage() {
  const { db, session } = await getServerContext({ roles: ["receptionist"], redirectTo: "/receptionist/login" });
  ...
  for (const row of consultationRows) {
    if (!latestByPatient.has(row.patient_id)) latestByPatient.set(row.patient_id, row.created_at);
  }
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="mb-4">
        <h2 className="text-2xl font-semibold tracking-tight text-[hsl(var(--text-primary))]">Patients</h2>
        <p className="text-sm text-[hsl(var(--text-muted))]">Reception view · scoped to this doctor</p>
      </div>
      <div className="mb-4">
        <ReceptionistIntakeCard patients={enriched.map((patient) => ({ id: patient.id, name: patient.name }))} />
      </div>
      <PatientsClient patients={enriched} />
    </div>
  );
}
